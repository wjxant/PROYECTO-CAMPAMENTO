...
function validarFormulario() {
    ...
    var campos = document.querySelectorAll('input[type="text"], input[type="email"], input[type="date"], textarea');

    campos.forEach(function(campo) {
        if (campo.nextElementSibling && campo.nextElementSibling.className === 'error') {
            valido = false;
        }
    });
    ...
    if (programas.length === 0) {
        mostrarError(document.getElementById('programa1'), 'Debe seleccionar al menos un programa');
        valido = false;
    }
    if (seguro === null) {
